...
create table customer (ID number, Name varchar2(100),City varchar2(100),industry_type varchar2(10));

insert into customer values(4, 'Samsonic','pleasant','J');
insert into customer values(4, 'Panasung','oaktown','J');
insert into customer values(4, 'Samony','jackson','B');
insert into customer values(4, 'Orange','Jackson','B');
...
select * from
(
SELECT S.id,"Number",Amount,
RANK() OVER(PARTITION BY salesperson_id ORDER BY Amount desc) as rank_
